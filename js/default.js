...
function getItem() {
    ...
    for (var i = 0; i < arrTabList.length; i++) {
        ...
        for (var j = 0; j < menudata.length; j++) {
            ...
            menuHtml += '" width=300px height=230px></td></tr>'
            menuHtml += '<tr><td><a1>'
...
function getSidemenuItem() {
    ...
    if (nowTabNum <= 1) {
        ...
        var menuHtml = '<h2>음료 추가</h2>';

        for (var i = 0; i < 6; i++) {
            if (i == 3) {
                menuHtml += '<h2>스낵 추가</h2>';
            }
...
function noClick() {
    $('#set_menu').addClass('clicknone');
    $('#hamburger').addClass('clicknone');
...
// 팝업창 닫으면 클릭되게 하기
function okClick() {
    $('#set_menu').removeClass('clicknone');
    $('#hamburger').removeClass('clicknone');
...
function show_cartList() {
    ...
    for (var i = 0; i < cartList.length; i++) {
        ...
        carthtml += '<td rowspan="2" style=\"width: 25%\"><img  style=\"width: 100px; height: 80px;\" src="'
        ...
        carthtml += '</td>';
        carthtml += '<td><input type="button" id="delete" value="삭제" onclick="delete_cartmenu(' + i + '); "></td>';
...
function choosePay(index) {
    ...
    setTimeout(function () {
        ...
        reHtml += ' <table><tr><th class="receipt_img" colspan="6"><img src="images/logo.jpg" width="200"></th></tr>'
        reHtml += '<tr><td class="tg-0pky" colspan="6"><a1>맥도날드 본리DT점</a1></td></tr>'
        reHtml += '<tr><td class="tg-0pky" colspan="6"><a1>대구광역시 달서구 와룡로 134</a1></td></tr>'
        ...
        reHtml += '<tr><td class="hansot" colspan="6">저희 맥도날드를</td></tr>';
        reHtml += '<tr><td class="hansot" colspan="6">이용해주셔서 감사합니다❤</td></tr></table>';
